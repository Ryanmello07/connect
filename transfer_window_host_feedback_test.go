...
func TestTcpReturnReplayPreservesPacingServiceAcrossFeedbackIdle(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		sequence.receiveSeq, sequence.receiveSeqAck = 101, 101
		sequence.receiveWindowSize = 1060
		// Packetize and retain the segment the way the socket reader does,
		// without delivering it: the source never acknowledges it, so the
		// replay below is the only packet that reaches the callback.
		var seqs []uint32
		var packets [][]byte
		func() {
			sequence.mutex.Lock()
			...
			var err error
			packets, err = sequence.DataPackets(make([]byte, 1060), 1060, settings.Mtu)
			if err != nil {
				t.Fatal(err)
			}
			sequence.retainReturnPacketsWithLock(packets)
			seqs = sequence.returnPacketSeqs(nil, packets)
			sequence.receiveSeq += 1060
		}()
		if len(seqs) != 1 {
			t.Fatalf("the segment packetized into %d packets", len(seqs))
		}
		for _, packet := range packets {
			MessagePoolReturn(packet)
		}
		sequence.markReturnDelivered(seqs, monotonicNanos())
		defer sequence.finishReturnRetransmit()
		done := make(chan struct{})
		go func() { defer close(done); sequence.runReturnRetransmitWorker() }()
