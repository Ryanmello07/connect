...
//
// FindExtension answers the FIRST entry of a type and a vector may legally hold two; refusing a
// repeat is ValSem209's job at whole leaf validation and not this accessor's, so a value coming
// back here does not mean the leaf carried exactly one.
//
...
func LeafKeysOf(leaf *LeafNode) (*LeafKeysExtension, error) {
	...
	for _, ext := range leaf.Extensions {
		...
		keys, err := ParseLeafKeysFrom(ext)
		if err != nil {
