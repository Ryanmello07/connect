...
// before.
func TestBusyProbeHoldsWhenSiblingPresentButSilent(t *testing.T) {
	stallTimeout := 20 * time.Millisecond
	...
	// a sibling exists but has no recent receive: uplink unproven
	silentSibling := stallTestChannel()
	window := busyProbeTestWindow(40*time.Millisecond, client, silentSibling)
