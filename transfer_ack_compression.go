...
type sequenceAckWindow struct {
	...
	// head below it has selective acks outstanding above it, whether or not
	// they were already written, so the next head advance is a hole filling
...
const (
	// enough distinct selective acks above the head are outstanding to prove
	// a hole to the sender (its SelectiveAckGapThreshold)
	gapWakeHoleProvable gapWakeReason = 1 << iota
	// a head ack advanced under selectively acked items: the hole they were
...
// signalGapWakeWithLock fires each reason once per cumulative head.
...
// GapWakeReasons reports the reasons signaled since the last cumulative head.
// The consumer reads it after taking a gap wake token; a reason it declines
// stays pending here, so it cannot signal again before the next head advance.
	...
	if !self.hasHeadAck || self.headAck.sequenceNumber < ack.sequenceNumber {
		if ack.selective {
			...
			if 0 < self.gapWakeSelectiveCount &&
				self.gapWakeSignaled&gapWakeHoleProvable == 0 &&
			...
			// all. New cumulative progress re-arms both reasons, and retires
			// only the evidence it absorbed: a selective ack still above the
			// new head is still evidence at the sender.
			self.gapWakeSignaled = 0
			self.gapEvidence = slices.DeleteFunc(self.gapEvidence,
				func(sequenceNumber uint64) bool {
					return sequenceNumber <= ack.sequenceNumber
				})
