...
//
// and it answers seal.go:5 and session.go:13 on this commit -- seventeen members before it and
// eighteen after. A reading that answered the same numbers with the door absent would be a reading
// of something else.
func TestTheEpochKeysDoorJoinsTheDerivedClassOfGroupSessionMethods(t *testing.T) {
	...
	if len(all) != 18 {
		t.Errorf("%d methods are declared on *GroupSession and this task's commit makes it 18; the number moves by one per method and Task 3's RebindServerNonce is the next one to move it",
			len(all))
	}
	if got := len(perFile["session.go"]); got != 13 {
		t.Errorf("session.go declares %d methods on *GroupSession and this task's commit makes it 13, which is the per-file half of the same count: the door belongs beside Epoch and AdvanceEpoch and not in seal.go",
			got)
	}
	if got := len(perFile["seal.go"]); got != 5 {
		t.Errorf("seal.go declares %d methods on *GroupSession and nothing in this task touches that file, so a change here is a method that landed in the wrong one",
			got)
	}
}
