...
// S10, the lossy-connection re-roll, on the path simulator of pathsim_test.go.
//
...
// The seed of every S10 arm. It fixes the hops' loss draws, the source port
...
// S10. A connection on a lossy path member is detected, re-rolled onto a new
...
//	S10/mode=off/leg=healthy                         300.8   303.0   18473       0    0
//	S10/mode=act/leg=healthy                         300.8   303.0   18473       0    0
//	S10/mode=off/leg=lossy                             5.0     4.9     856      14   14
//	S10/mode=observe/leg=lossy                         5.0     4.9     856      14   14
//	S10/mode=act/leg=lossy/hash=independent/port=far  227.0   302.5   27723     204    2
//	S10/mode=act/leg=lossy/ack=measured              134.0   263.0   24777     191   13
//	S10/mode=off/path=same-dc                        993.2   993.5   15338       0    0
//	S10/mode=act/path=same-dc                        993.2   993.5   15338       0    0
//
...
//
//	S10/mode=act/leg=lossy/hash=block64/port=kernel     4.6     5.1    2352     409  207
//	S10/mode=act/leg=lossy/hash=block64/port=far      281.6   301.8  128923     204    2
//	S10/mode=act/leg=lossy/ooo=third                  281.6   301.8  128923     204    2
//	S10/mode=act/leg=lossy/ooo=unknown                  4.6     5.1    2352     409  207
//	S10/mode=act/leg=lossy/members=1/ack=measured       4.9     5.3    2480     417   60
//
...
func TestPathsimS10LossyConnectionReroll(t *testing.T) {
	...
	const off = "S10/mode=off/leg=lossy"
	const observe = "S10/mode=observe/leg=lossy"
	const act = "S10/mode=act/leg=lossy/hash=independent/port=far"
	const healthyOff = "S10/mode=off/leg=healthy"
	const healthyAct = "S10/mode=act/leg=healthy"
	const shortOff = "S10/mode=off/path=same-dc"
	const shortAct = "S10/mode=act/path=same-dc"
	const measured = "S10/mode=act/leg=lossy/ack=measured"
	...
	const blockKernel = "S10/mode=act/leg=lossy/hash=block64/port=kernel"
	const blockFar = "S10/mode=act/leg=lossy/hash=block64/port=far"
	const thirdTickOoo = "S10/mode=act/leg=lossy/ooo=third"
	const unknownOoo = "S10/mode=act/leg=lossy/ooo=unknown"
	const everyMemberLossy = "S10/mode=act/leg=lossy/members=1/ack=measured"
	if pathsimFull() {
	...
	reportPathScenario(t, "S10 lossy connection re-roll", ordered)
	for _, name := range order {
		result := results[name]
		...
		t.Logf(
			"S10: %s ports=%v convictions=%d(first %s) switches=%d(first %s) improved=%d recovery=%s switchdrops=%d"+
	...
	if pathRerollCollapsedRate < offResult.steadyGoodput() {
		t.Errorf(
			"S10: %s read %.1f Mb/s, not the collapse the lossy member is supposed to be",
	...
	if float64(offResult.writeCount)*pathRerollCollapseResendShare < float64(offResult.resendCount) {
		t.Errorf(
			"S10: %s resent %d of %d writes; the collapse is supposed to be nearly invisible to Transfer",
	...
	if observeResult.reroll.convictions < 1 {
		t.Errorf("S10: %s convicted %d times", observe, observeResult.reroll.convictions)
	}
	if first := observeResult.reroll.firstConvictionAfter; first < 2500*time.Millisecond || 4500*time.Millisecond < first {
		t.Errorf(
			"S10: %s convicted first at %s, outside the design's ~3.5 s",
			observe, formatPathDuration(first),
		)
	}
	if observeResult.reroll.switches != 0 {
		t.Errorf("S10: %s switched %d times", observe, observeResult.reroll.switches)
	}
	assertPathRerollEqual(t, "S10", offResult, observeResult, "publishing the observer")

	// Act rescues the connection.
	if actResult.reroll.switches != 1 {
		t.Errorf("S10: %s switched %d times, want exactly one", act, actResult.reroll.switches)
	}
	if first := actResult.reroll.firstSwitchAfter; first < 2500*time.Millisecond || 5*time.Second < first {
		t.Errorf("S10: %s switched first at %s", act, formatPathDuration(first))
	}
	if actResult.reroll.convictions != 1 {
		t.Errorf(
			"S10: %s convicted %d times; the replacement connection is supposed to be healthy",
			act, actResult.reroll.convictions,
		)
	}
	if actResult.reroll.improved != 1 {
		t.Errorf("S10: %s resolved %d re-rolls as improved", act, actResult.reroll.improved)
	}
	if !actResult.drained || actResult.duplicates < 0 {
		t.Errorf(
			"S10: %s did not drain, so its duplicate count is not a reading (%d)",
			act, actResult.duplicates,
		)
	}
	if rescued := pathRatio(actResult, healthyActResult); rescued < 0.6 {
		t.Errorf(
			"S10: %s read %.1f Mb/s steady, %.2f of the healthy %.1f Mb/s; the re-roll did not rescue it",
	...
	if measuredResult.reroll.returned < 10 {
		t.Errorf(
			"S10: %s sent %d upstream messages; the arm has no return direction to measure",
			measured, measuredResult.reroll.returned,
		)
	}
	if measuredResult.reroll.ackTicks == 0 {
		t.Errorf("S10: %s read no ack round trip of its own", measured)
	}
	if ackRtt := measuredResult.reroll.maxAckRttBeforeSwitch; ackRtt < time.Second {
		t.Errorf(
			"S10: %s convicted with an ack round trip of %s; the collapse is supposed to hold the acks too",
	...
	if steadyResends := measuredResult.reroll.resendsBeforeSwitch - measuredResult.reroll.resendsAtSteady; steadyResends != 0 {
		t.Errorf(
			"S10: %s resent %d packs while the collapse stood (%d of %d writes in all); the rig read none",
	...
	if measuredResult.reroll.writesBeforeSwitch <= measuredResult.reroll.writesAtSteady {
		t.Errorf(
			"S10: %s wrote nothing between the steady mark and the switch, so its resend count says nothing",
			measured,
		)
	}
	if measuredResult.reroll.switches != 1 || measuredResult.reroll.convictions != 1 ||
		...
		t.Errorf(
			"S10: %s convicted %d times, switched %d times and improved %d, want one of each",
	...
	if recovery := measuredResult.reroll.recoveryAfterSwitch; recovery == 0 || 2*time.Second < recovery {
		t.Errorf(
			"S10: %s recovered the healthy rate %s after the switch",
			measured, formatPathDuration(recovery),
		)
	}
	if rescued := pathRatio(measuredResult, healthyActResult); rescued < 0.6 {
		t.Errorf(
			"S10: %s read %.2f of the healthy rate after its re-roll",
			measured, rescued,
		)
	}
	...
	if 0 < healthyActResult.reroll.convictions || 0 < healthyActResult.reroll.switches {
		t.Errorf(
			"S10: %s convicted %d times and switched %d times on a healthy member",
			healthyAct, healthyActResult.reroll.convictions, healthyActResult.reroll.switches,
		)
	}
	assertPathRerollEqual(t, "S10", healthyOffResult, healthyActResult, "monitoring a healthy connection")
	...
	if shortActResult.reroll.dormant != 1 || shortActResult.reroll.monitored != 0 {
		t.Errorf(
			"S10: %s monitored %d connections and made %d dormant on a %s path",
	...
	if 0 < shortActResult.reroll.convictions {
		t.Errorf("S10: %s convicted %d times", shortAct, shortActResult.reroll.convictions)
	}
	assertPathRerollEqual(t, "S10", shortOffResult, shortActResult, "Act on a same-datacenter path")
	...
	if blockKernelResult.reroll.switches != 1 {
		t.Errorf(
			"S10: %s switched %d times, want the one the unconfirmed budget allows",
			blockKernel, blockKernelResult.reroll.switches,
		)
	}
	if blockKernelResult.reroll.improved != 0 {
		t.Errorf(
			"S10: %s credited %d re-rolls as improved while it stayed on the bad block",
			blockKernel, blockKernelResult.reroll.improved,
		)
	}
	if blockKernelResult.reroll.suppressedUnconfirmedBudget < 1 {
		t.Errorf("S10: %s never refused a conviction for the unconfirmed budget", blockKernel)
	}
	if latched := pathRatio(blockKernelResult, healthyActResult); 0.2 < latched {
		t.Errorf(
			"S10: %s read %.2f of the healthy rate; it is supposed to stay on the bad block",
			blockKernel, latched,
		)
	}
	...
	if blockFarResult.reroll.switches != 1 {
		t.Errorf("S10: %s switched %d times, want exactly one", blockFar, blockFarResult.reroll.switches)
	}
	if rescued := pathRatio(blockFarResult, healthyActResult); rescued < 0.6 {
		t.Errorf(
			"S10: %s read %.2f of the healthy rate; the far-random port did not leave the block",
			blockFar, rescued,
		)
	}
	...
	if thirdResult.reroll.switches != 1 {
		t.Errorf("S10: %s switched %d times, want exactly one", thirdTickOoo, thirdResult.reroll.switches)
	}
	if thirdResult.reroll.confirmedConvictions != 0 ||
		thirdResult.reroll.unconfirmedConvictions < 1 {
		t.Errorf(
			"S10: %s convicted %d confirmed and %d unconfirmed with no ack on the route",
	...
	if rescued := pathRatio(thirdResult, healthyActResult); rescued < 0.6 {
		t.Errorf(
			"S10: %s read %.2f of the healthy rate; one tick in three of loss evidence did not convict it",
	...
	if unknownResult.reroll.confirmedConvictions != 0 {
		t.Errorf(
			"S10: %s confirmed %d convictions with no kernel counter to confirm them",
			unknownOoo, unknownResult.reroll.confirmedConvictions,
		)
	}
	if unknownResult.reroll.suppressedUnconfirmedBudget < 1 {
		t.Errorf("S10: %s never refused a conviction for the unconfirmed budget", unknownOoo)
	}
	...
	if everyMemberResult.reroll.switches != settings.MaxUnimprovedRerolls {
		t.Errorf(
			"S10: %s switched %d times, want the %d the unimproved latch allows",
			everyMemberLossy, everyMemberResult.reroll.switches, settings.MaxUnimprovedRerolls,
		)
	}
	if everyMemberResult.reroll.improved != 0 {
		t.Errorf(
			"S10: %s credited %d re-rolls as improved while every replacement stayed collapsed",
	...
	if everyMemberResult.reroll.confirmedConvictions == 0 ||
		...
		t.Errorf(
			"S10: %s convicted %d confirmed and refused %d for the unconfirmed budget; the latch is supposed to be what binds",
	...
	if everyMemberResult.reroll.suppressedLatched == 0 {
		t.Errorf("S10: %s never latched the epoch", everyMemberLossy)
	}
	if latched := pathRatio(everyMemberResult, healthyActResult); 0.2 < latched {
		t.Errorf(
			"S10: %s read %.2f of the healthy rate; every member of this path is the lossy one",
...
// integer of a run. Every scenario but S10 runs without controls, so this is
